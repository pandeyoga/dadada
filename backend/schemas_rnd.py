...
class SampleInput(BaseModel):
    spec_id: str = ""
    # Konsolidasi: spesifikasi produk boleh langsung diisi saat membuat permintaan sample
    # (tanpa membuka tab Spesifikasi terpisah). Diabaikan bila `spec_id` sudah diisi.
...
class SampleDecideBody(BaseModel):
    ...
    valid_to: str = ""
    # Diisi SAAT sampling ACC (bukan di awal): nama & kode warna versi supplier → disinkronkan ke pustaka warna.
    supplier_color_name: str = ""
    supplier_color_code: str = ""
    # Sekaligus ACC spesifikasi → master produk (SKU) lahir sebagai varian induk / produk konsep.
