...
async def create_customer(payload: CustomerCreate, request: Request) -> Dict[str, Any]:
    ...
    # REVISI KLIEN 17 Sep — PIC TOKO (pihak pelanggan) & alamat toko WAJIB; alamat "-" tidak diterima.
    if not (payload.pic_name or "").strip():
        raise HTTPException(status_code=400, detail="Nama PIC toko (pihak pelanggan) wajib diisi.")
    if not (payload.phone or "").strip():
        raise HTTPException(status_code=400, detail="Nomor HP/WhatsApp PIC toko wajib diisi.")
    if len((payload.address or "").strip().strip("-").strip()) < 3:
        raise HTTPException(status_code=400, detail="Alamat toko wajib diisi.")
    cust_code = await _ndn("customers", "code", "CUST-", width=4)   # D-01: atomik, bukan count()+1
    # assigned_sales: sales pembuat SELALU jadi pemilik (login = identitas, tak perlu memilih);
    # peran lain (admin/manager/sales_admin) WAJIB memilih sales penanggung jawab.
    assigned_id = payload.assigned_sales_id
    if actor.get("role") == "sales":
        assigned_id = actor["id"]
    team_pic = pic_of(payload.sales_team and [m.model_dump() for m in payload.sales_team] or [])
    if not assigned_id and not (team_pic or {}).get("sales_id"):
        raise HTTPException(status_code=400, detail="Pilih Sales penanggung jawab (PIC internal) untuk pelanggan ini.")
    assigned_name = ""
    if assigned_id:
