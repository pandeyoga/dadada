...
async def form_options(project_id: str = None,
    ...
    schemes = await db.payment_schemes.find(
        {"org_id": org, "active": {"$ne": False}},
        {"_id": 0, "id": 1, "name": 1, "type": 1, "kind": 1, "code": 1, "is_default": 1,
         "dp_pct": 1, "installments": 1, "items": 1}).sort("created_at", 1).to_list(50)
    for s in schemes:
        kind = s.get("kind") or s.get("type")
        s["kind"] = kind
        s["kind_label"] = ref.label_of("payment_scheme_kind", kind or "") if kind else None
        s["items"] = [{"label": i.get("label"), "basis": i.get("basis"), "value": i.get("value"),
