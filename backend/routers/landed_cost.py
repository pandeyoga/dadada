...
from entity_scope import assert_entity_access, entity_ctx, resolve_list_scope, guard_doc
...
async def submit_landed_cost(voucher_id: str, request: Request) -> Dict[str, Any]:
    ...
    await guard_doc(request, "landed_cost_vouchers", await db.landed_cost_vouchers.find_one({"id": voucher_id}, {"_id": 0}),
                    not_found="Landed Cost Voucher tidak ditemukan")  # KN-B33
...
async def approve_landed_cost(voucher_id: str, request: Request) -> Dict[str, Any]:
    ...
    v = await db.landed_cost_vouchers.find_one({"id": voucher_id}, {"_id": 0})
    await guard_doc(request, "landed_cost_vouchers", v, not_found="Landed Cost Voucher tidak ditemukan")  # KN-B33
