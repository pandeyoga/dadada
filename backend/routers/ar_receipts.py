...
async def create_receipt(payload: ReceiptPayload, request: Request,
    ...
    # KN-B29 — entity_id dari body diadu dengan penugasan (403 bila di luar).
    body["entity_id"] = resolve_requested_entity(ctx, body.get("entity_id"))
...
async def void_receipt(receipt_id: str, request: Request,
    ...
    await guard_doc(request, "ar_receipts", await _db.ar_receipts.find_one({"id": receipt_id}, {"_id": 0}),
                    not_found="Kwitansi tidak ditemukan")  # KN-B29
