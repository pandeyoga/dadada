...
async def approve_order(order_id: str, request: Request) -> Dict[str, Any]:
    ...
    # KN-D02 — persetujuan yang DITOLAK (kredit/harga khusus) memblokir jalur cadangan
    # `_transition` juga; dulu hanya `all_approved()` yang menghormatinya sehingga SO yang
    # permintaan kreditnya ditolak tetap bisa 'approved' dan mengunci stok.
    if summ.get("has_rejected"):
        raise HTTPException(status_code=409, detail={
            "code": "APPROVAL_REJECTED",
            "message": "Ada permintaan persetujuan yang DITOLAK pada pesanan ini — pesanan tidak bisa "
                       "disetujui. Ubah pesanan (harga/limit) lalu ajukan ulang."})
    pa = list(order.get("pending_approvals") or [])
    for p in pa:
