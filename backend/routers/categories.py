...
@router.get("/product-motifs")
async def list_motifs(request: Request) -> Dict[str, Any]:
    """Pilihan motif tanpa ketik bebas: Polos + motif yang sudah dipakai induk/SKU + Master Desain (motif)."""
    await require_permission(request, "product", "view")
    seen: Dict[str, Dict[str, str]] = {"polos": {"value": "Polos", "label": "Polos", "source": "baku"}}
    for coll, src in (("product_templates", "induk"), ("products", "sku")):
        for m in await db[coll].distinct("motif"):
            key = str(m or "").strip()
    ...
    items = sorted(seen.values(), key=lambda x: (x["value"] != "Polos", x["value"].casefold()))
    return {"items": items}
