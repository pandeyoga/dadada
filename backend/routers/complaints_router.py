...
from core_utils import now_iso, serialize_doc, parse_pagination, new_id, due_in
from rbac import has_role, require_permission, audit_log, is_scoped_sales
from engine import create_notification, auto_create_task
from notifications import send_whatsapp
from models import ComplaintRespond, ComplaintStatusUpdate, ComplaintAssign, ComplaintManualCreate

router = APIRouter(prefix="/complaints", tags=["complaints"])

STATUSES = ("open", "in_progress", "resolved")
CHANNELS = ("whatsapp", "telepon", "walk_in", "email", "lainnya")


@router.post("")
async def create_manual_complaint(payload: ComplaintManualCreate,
                                  user: dict = Depends(require_permission("complaints", "create"))):
    """Komplain dicatat MANUAL oleh manajemen (pembeli tidak semua memakai portal).

    Alurnya sama persis dengan komplain portal: SLA 48 jam, tugas untuk PIC, notifikasi,
    dan (opsional) WA konfirmasi ke pembeli — hanya sumbernya yang berbeda (`source=manual`).
    """
    org = user.get("org_id", ORG_ID)
    if not (payload.subject or "").strip() or not (payload.message or "").strip():
        raise HTTPException(status_code=400, detail="Subjek dan isi komplain wajib diisi.")
    if payload.channel not in CHANNELS:
        raise HTTPException(status_code=400, detail="Kanal komplain tidak dikenal.")
    cust = await db.customers.find_one({"id": payload.customer_id, "org_id": org}, {"_id": 0})
    if not cust:
        raise HTTPException(status_code=404, detail="Pelanggan tidak ditemukan.")
    ors = [{"customer_id": cust["id"]}]
    if cust.get("lead_id"):
        ors.append({"lead_id": cust["lead_id"]})
    deals = await db.deals.find({"org_id": org, "$or": ors,
    ...
    if payload.deal_id:
        ...
        deal = next((d for d in deals if d.get("unit_id") == payload.unit_id), None)
    elif deals:
        deal = deals[0]
    if is_scoped_sales(user) and deal and deal.get("assigned_to") != user.get("email"):
        raise HTTPException(status_code=403, detail="Akses ditolak: bukan pembeli Anda")
    unit_id = payload.unit_id or (deal or {}).get("unit_id")
    unit = await db.units.find_one({"id": unit_id}, {"_id": 0, "code": 1}) if unit_id else None
    ts = now_iso()
    cid = new_id()
    sla = due_in(hours=48)
    assignee = payload.assigned_to or (deal or {}).get("assigned_to") or user.get("email")
    doc = {
        "id": cid, "org_id": org, "customer_id": cust["id"], "customer_name": cust.get("name"),
        "deal_id": (deal or {}).get("id"), "unit_code": (unit or {}).get("code"),
        "unit_id": unit_id, "category": payload.category or "umum",
        "subject": payload.subject.strip(), "message": payload.message.strip(),
        "status": "open", "priority": payload.priority or "medium",
        "assigned_to": assignee, "sla_due_at": sla, "responses": [],
        "source": "manual", "channel": payload.channel, "created_by": user.get("email"),
        "created_at": ts, "updated_at": ts,
    }
    doc.update(await clock.patch_for("complaint", "open", org_id=org, at=ts))
    await db.complaints.insert_one(dict(doc))
    await auto_create_task(
        source_event=f"complaint:{cid}", title=f"Komplain pembeli: {doc['subject']}",
        jobdesk_code="SM-09", type="complaint", related_entity_type="complaint",
        related_entity_id=cid, assigned_to=assignee, sla_due_at=sla, priority=doc["priority"],
        description=f"{cust.get('name')} ({doc['unit_code'] or '-'}) via {payload.channel}: "
                    f"{doc['message']}", org_id=org)
    if assignee and assignee != user.get("email"):
        await create_notification(user_email=assignee, title="Komplain pembeli baru (dicatat manual)",
                                  body=f"{cust.get('name')}: {doc['subject']}", type="complaint",
                                  related_entity_type="complaint", related_entity_id=cid, org_id=org)
    if payload.notify_customer and cust.get("phone"):
        await send_whatsapp(cust["phone"],
                            f"Terima kasih. Komplain Anda '{doc['subject']}' telah kami catat dan "
                            "akan ditindaklanjuti (SLA 2x24 jam).")
    await audit_log(user, "create", "complaints", cid, {"source": "manual", "channel": payload.channel})
    return {"data": serialize_doc(_mark_breach(doc, ts))}
