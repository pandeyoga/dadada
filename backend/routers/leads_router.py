...
async def lead_assignees(user: dict = Depends(require_permission("leads", "view"))):
    """Calon PIC lead: pengguna aktif berperan sales/manajer/marketing (untuk form Tambah Lead).

    Berbeda dari `/leads/owners` (yang hanya menyebut orang yang SUDAH memegang lead),
    daftar ini memuat siapa saja yang BOLEH diberi lead — termasuk sales baru tanpa lead.
    """
    org = user.get("org_id", ORG_ID)
    if is_scoped_sales(user):
        return {"data": [{"value": user.get("email"), "label": user.get("name") or user.get("email"),
                          "role": user.get("role")}], "total": 1}
    people = await db.users.find(
        {"org_id": org, "is_active": {"$ne": False},
         "role": {"$in": ["sales", "sales_manager", "marketing_admin"]}},
        {"_id": 0, "email": 1, "name": 1, "role": 1}).sort("name", 1).to_list(300)
    out = [{"value": p["email"], "label": p.get("name") or p["email"], "role": p.get("role")}
           for p in people]
