...
from entity_scope import EntityContext, entity_ctx, resolve_list_scope, guard_doc
...
async def _guard(return_id: str, request: Request) -> Dict[str, Any]:
    """KN-A05 — seluruh siklus retur beli dijaga entitas sebelum service dipanggil."""
    doc = safe_doc(await db.purchase_returns.find_one({"id": return_id}, {"_id": 0}))
    await guard_doc(request, "purchase_returns", doc, not_found="Retur tidak ditemukan")
    return doc
...
async def list_purchase_returns(
    ...
    # KN-A06 — entity_id diadu dengan penugasan; tanpa parameter → entitas AKTIF (bukan semua PT).
    q = resolve_list_scope("purchase_returns", q, await entity_ctx(request), entity_id)
...
async def purchase_return_status_counts(
    ...
    q = resolve_list_scope("purchase_returns", {}, await entity_ctx(request), entity_id)  # KN-A06
...
async def create_purchase_return(payload: PurchaseReturnCreate, request: Request) -> Dict[str, Any]:
    ...
    if payload.entity_id:  # KN-A05 — entitas dari body wajib ∈ penugasan
        from entity_scope import resolve_requested_entity
        payload.entity_id = resolve_requested_entity(await entity_ctx(request), payload.entity_id)
...
async def get_purchase_return(return_id: str, request: Request) -> Dict[str, Any]:
    await require_permission(request, "purchase_return", "view")
    return await _guard(return_id, request)
...
async def submit_purchase_return(return_id: str, request: Request) -> Dict[str, Any]:
    ...
    await _guard(return_id, request)
    try:
