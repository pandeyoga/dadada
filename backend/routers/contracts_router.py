...
@router.get("/contracts/{contract_id}/kpr/plafon-check")
async def kpr_plafon_check(contract_id: str, plafon: int = 0,
                           user: dict = Depends(require_permission("financing", "view"))):
    """Pratinjau plafon SP3K vs porsi bank pada AR: selisih kurang → kewajiban pembeli; lebih → porsi bank naik."""
    import kpr_plafon as kp
    c = await _contract_scoped(contract_id, user)
    inv = await db.ar_invoices.find_one({"org_id": _org(user), "deal_id": c["deal_id"]}, {"_id": 0, "items": 1})
    if not inv:
        return {"data": {"has_ar": False, "has_bank_items": False, "kind": "no_ar", "plafon": plafon,
                         "bank_portion": 0, "diff": 0, "shortfall": 0, "excess": 0,
                         "requires_due_date": False, "excess_ok": True, "shortfall_ok": True}}
    return {"data": {"has_ar": True, **kp.preview(inv, plafon)}}
