...
def _spec_body_from_sample(spec_body: Dict[str, Any], sample_body: Dict[str, Any]) -> Dict[str, Any]:
    """Spesifikasi lahir bersama sample: judul/desain/pesanan/lini/warna diwariskan dari permintaan bila kosong."""
    if not (spec_body.get("title") or "").strip():
        spec_body["title"] = sample_body.get("title") or ""
    for k in ("design_id", "so_id", "customer_id", "line_code"):
        if not spec_body.get(k) and sample_body.get(k):
            spec_body[k] = sample_body[k]
    if not (spec_body.get("color_target") or {}).get("color_id") and (sample_body.get("color_target") or {}).get("color_id"):
        spec_body["color_target"] = sample_body["color_target"]
    if not spec_body.get("base_unit") and sample_body.get("unit"):
        spec_body["base_unit"] = sample_body["unit"]
    types = sample_body.get("sample_types") or []
    if types:
        spec_body["sample_type_hint"] = types[0]
    return spec_body


def _validate_spec_for_types(spec_body: Dict[str, Any], sample_types: List[str]) -> None:
    """Syarat spesifikasi per jenis: labdip → warna target; proofing → desain; semua → jenis kain."""
    types = [str(t).lower() for t in (sample_types or [])]
    target = spec_body.get("target") or {}
    if not (target.get("fabric_type") or "").strip():
        raise RndError("Spesifikasi: jenis kain (woven/knit) wajib diisi.")
    if "labdip" in types and not (spec_body.get("color_target") or {}).get("color_id"):
        raise RndError("Spesifikasi labdip: warna target dari Pustaka Warna wajib dipilih.")
    if "proofing" in types and not spec_body.get("design_id"):
        raise RndError("Spesifikasi proofing: desain/pattern wajib dipilih.")
...
def _assert_appraisal_role(actor: Dict[str, Any]) -> None:
    ...
    if not reg.role_in((actor or {}).get("role"), APPRAISAL_ROLES):
        raise HTTPException(
            status_code=403,
...
async def approve_spec(spec_id: str, payload: SpecApproveBody, request: Request) -> Dict[str, Any]:
    ...
    try:
        ...
        if (res.get("product") or {}).get("id"):
            from services import color_service as _colors
            await _colors.sync_product_colors(res["product"]["id"])
    except RndError as exc:
        raise _err(exc) from exc
...
async def list_samples(request: Request, entity_id: Optional[str] = Query(None),
    ...
    await smp.attach_spec_summaries(rows)
    return {"count": len(rows), "items": rows, "stats": await smp.stats(scope)}


@router.post("/rnd/samples/{sample_id}/spec")
async def attach_sample_spec(sample_id: str, payload: SpecInput, request: Request) -> Dict[str, Any]:
    """Lengkapi spesifikasi untuk sample lama yang belum punya (1 sample = 1 spesifikasi)."""
    actor = await require_permission(request, "rnd", "create")
    ctx = await entity_ctx(request)
    doc = await _sample_guard(sample_id, ctx)
    if doc.get("spec_id"):
        raise HTTPException(status_code=400, detail=f"Sample ini sudah punya spesifikasi {doc.get('spec_number') or ''}.")
    try:
        spec_body = _spec_body_from_sample(payload.model_dump(), doc)
        _validate_spec_for_types(spec_body, doc.get("sample_types") or [])
        spec_doc = await spec_svc.create_spec(spec_body, entity_id=doc.get("entity_id") or ctx.active_entity_id,
                                              actor=actor.get("name", ""))
        return await smp.link_spec(sample_id, spec_doc, actor=actor.get("name", ""))
    except RndError as exc:
        raise _err(exc) from exc
...
async def create_sample(payload: SampleInput, request: Request) -> Dict[str, Any]:
    ...
    try:
        if not payload.spec_id and not payload.spec:
            raise RndError("Spesifikasi produk WAJIB — setiap permintaan sample (labdip/handfeel/proofing) "
                           "lahir bersama spesifikasi target-nya. Isi bagian Spesifikasi atau pilih spesifikasi acuan.")
        if payload.spec and not payload.spec_id:
            spec_body = _spec_body_from_sample(payload.spec.model_dump(), body)
            _validate_spec_for_types(spec_body, payload.sample_types)
            spec_doc = await spec_svc.create_spec(spec_body, entity_id=ctx.active_entity_id, actor=actor.get("name", ""))
            body["spec_id"] = spec_doc["id"]
        body.pop("spec", None)
        doc = await smp.create_sample(body, entity_id=ctx.active_entity_id,
                                      actor=actor.get("name", ""))
    except RndError as exc:
        raise _err(exc) from exc
...
async def get_sample(sample_id: str, request: Request) -> Dict[str, Any]:
    ...
    doc["master_data"] = await smp.master_data_of(doc)
    return doc
...
async def designer_kpi_trend_report(request: Request,
                                    ...
                                    metric: str = Query("grade", description="grade|avg_score|design_score"),
                                    ) -> Dict[str, Any]:
