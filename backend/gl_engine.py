...
Posting map (Indonesian property developer, PSAK-aligned; contract liability model):
  ...
  ap.approved          Dr 1-1400/1-1600/6-1300   / Cr 2-1100 Utang Usaha (+ 2-1200 Retensi)
                       (WIP: akrual 2-1700 dilunasi lebih dulu bila ada)
  ap.paid              Dr 2-1100 Utang Usaha      / Cr 1-1200 Bank
  commission.approved  Dr 6-1100 Beban Komisi     / Cr 2-1600 Utang Komisi
  revenue.recognized   Dr 2-1400 + Dr 1-1300 AR   / Cr 4-1100 Pendapatan ; Dr 5-1100 HPP / Cr 1-1600 WIP (+ 2-1700 akrual)
...
STANDARD_COA = [
    ...
    # HPP yang diakui saat BAST (dari RAB) di atas WIP yang sudah dikapitalisasi = biaya
    # konstruksi yang masih akan ditagih subkon/vendor. Dilunasi oleh tagihan WIP berikutnya.
    ("2-1700", "Utang Biaya Konstruksi Ditaksir (Akrual HPP)", "liability"),
...
async def _gl_deposit_applied(ev):
    ...
    if amt <= 0:
        return
    # Deal yang sudah DIHAPUS PAKSA sebelum peristiwa ini diproses tidak boleh meninggalkan
    # jurnal yatim (2-1450/2-1400 tidak lagi cocok dengan Σ titipan / Σ kewajiban kontrak).
    did = (ev.get("data") or {}).get("deal_id")
    if did and not await db.deals.find_one({"id": did}, {"_id": 0, "id": 1}):
        return
...
async def _gl_ap_approved(ev):
    ...
    if dr_code == "1-1600":
        # Biaya konstruksi yang HPP-nya sudah diakui saat BAST (akrual 2-1700) dilunasi dulu.
        import revrec_cogs as rc
        lines = await rc.accrual_debit_lines(org, claimed)
    else:
        lines = [{"account_code": dr_code, "debit": claimed, "credit": 0}]
    lines.append({"account_code": "2-1100", "debit": 0, "credit": net})
    if ret > 0:
...
async def _gl_revenue_recognized(ev):
    ...
    # Uang diterima melebihi pendapatan (mis. AR pernah disusun ulang lebih kecil): kelebihan
    # menjadi titipan pelanggan yang harus dikembalikan — bukan dihilangkan agar jurnal "pas".
    if cleared > rev:
        lines.append({"account_code": "2-1450", "debit": 0, "credit": cleared - rev,
                      "memo": "Kelebihan penerimaan di atas pendapatan yang diakui"})
    if cogs > 0:
        import revrec_cogs as rc
        lines.append({"account_code": "5-1100", "debit": cogs, "credit": 0})
        lines.extend(await rc.cogs_credit_lines(org, cogs))
