...
DEFAULTS: dict = {d["key"]: d for d in [
    ...
    _d("kpr.disbursement.require_scheme", True, "bool", "kpr",
       "Pencairan KPR wajib mengikuti skema pencairan",
       "Bila MENYALA, pencairan bank hanya boleh dicatat dari TAHAPAN skema pencairan "
       "(Pusat Konfigurasi › Pencairan KPR) — nominal bebas ditolak. Skema bawaan dipasang "
       "otomatis saat SP3K tercatat bila kontrak belum memilih skema.",
       impact="Mematikan berarti finance boleh mencatat nominal pencairan bebas tanpa tahapan.",
       sensitive=True),
    _d("kpr.disbursement.default_scheme_code", "BANK_STD", "str", "kpr",
       "Kode skema pencairan bawaan",
       "Skema yang dipasang otomatis pada kontrak KPR saat SP3K tercatat (bisa diganti sales/"
       "finance selama belum ada pencairan)."),
    ...
    _d("pricing.approval_min_amount", 0, "int", "penawaran",
       "Ambang nominal potongan wajib persetujuan (Rp, 0 = mati)",
       "Bila TOTAL potongan (harga, DP, booking fee, biaya all-in) mencapai nominal ini, penawaran "
       "maupun reservasi (SPR) WAJIB disetujui manajer — melengkapi ambang persen di atas. Mode "
       "persetujuan per skema diskon/promo/kupon ('selalu' / 'tidak pernah') menimpa ambang global.",
       impact="0 berarti hanya ambang persen dan mode per aturan yang berlaku.",
