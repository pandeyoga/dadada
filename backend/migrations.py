...
async def bank_portion_items() -> dict:
    ...
    # AR yang lahir SEBELUM kontrak KPR-nya (payer sudah terisi 'buyer' semua) → tandai ulang.
    async for inv in db.ar_invoices.find({"deal_id": {"$in": list(kpr_deals)}, "items.payer": {"$ne": "bank"}},
                                         {"_id": 0, "org_id": 1, "deal_id": 1}):
        if await fe.tag_bank_items_for_deal(inv["org_id"], inv["deal_id"]):
            n += 1
