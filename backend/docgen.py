...
async def _spr_gates(org: str, contract: dict) -> list:
    ...
    blocks = []
    deal = await db.deals.find_one({"id": contract.get("deal_id")}, {"_id": 0}) or {}
    # Potongan reservasi langsung (jalur SPR) yang belum/tidak disetujui manajer menahan SPR.
    pa = deal.get("pricing_approval") or {}
    if pa.get("state") == "pending":
        blocks.append(("potongan_menunggu_persetujuan",
                       "Potongan pada reservasi ini masih menunggu persetujuan manajer."))
    elif pa.get("state") == "rejected":
        blocks.append(("potongan_ditolak",
                       f"Potongan reservasi DITOLAK manajer ({pa.get('decision_reason') or '-'}) — "
                       "batalkan reservasi dan buat ulang tanpa potongan tersebut."))
