...
async def tag_bank_items_for_deal(org_id: str, deal_id: str) -> bool:
    """AR deal KPR yang belum menandai porsi bank (mis. AR lahir sebelum kontrak KPR-nya) →
    tandai termin berlabel KPR/pencairan/bank. Idempoten; True bila ada yang ditandai."""
    inv = await db.ar_invoices.find_one({"org_id": org_id, "deal_id": deal_id}, {"_id": 0, "id": 1, "items": 1})
    if not inv or any(is_bank_item(i) for i in inv.get("items") or []):
        return False
    items = infer_bank_items([dict(i) for i in inv.get("items") or []], True)
    if not any(is_bank_item(i) for i in items):
        return False
    await db.ar_invoices.update_one({"id": inv["id"]}, {"$set": {"items": items, "updated_at": now_iso()}})
    return True
...
async def create_ar_for_deal(deal: dict, scheme_id=None, org_id=ORG_ID, replace=False, actor="system") -> dict:
    ...
    # Bila skema all-in deal memuat komponen BOOKING, booking fee justru melunasi komponen
    # itu (titipan biaya) — bukan termin unit. Sisanya (jika ada) tetap ke termin.
    import allin_engine as _ae
    try:
        await _ae.apply_booking_deposit(org_id, deal, actor)
    except Exception:  # noqa: BLE001
        logger.exception("Pengalihan booking fee ke komponen BOOKING gagal untuk deal %s", deal_id)
...
async def _deposit_move(org_id, deal_id, inv, kind, amount, note, actor, receipt_id=None,
                        cash_account_id=None, proof_file_ids=None) -> dict:
    """Catat satu mutasi titipan (in/apply/apply_cost/refund) + jurnal GL. Return dokumen titipan terkini.
    `apply_cost` = titipan dialihkan ke komponen biaya all-in (jurnalnya dipos langsung pemanggil)."""
    amount = int(amount)
    ts = now_iso()
    ...
    inc = {"balance": delta,
           ...
           "applied_total": amount if kind in ("apply", "apply_cost") else 0,
...
async def receive_deposit(deal_id, amount, note, actor, org_id=ORG_ID, cash_account_id=None,
                          proof_file_ids=None) -> dict:
    ...
    dep = await _deposit_move(org_id, deal_id, ctx, "in", amount,
                              ...
                              cash_account_id=cash_account_id, proof_file_ids=proof_file_ids)
