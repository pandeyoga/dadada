...
async def next_return_number() -> str:
    ...
    return await next_doc_number("purchase_returns", "number", "PRET-", width=5, scheme="shared")


async def next_debit_note_number() -> str:
    """KN-A12 — sequence ATOMIK bersama (find_one_and_update $inc), bukan 'nomor tertinggi + 1'."""
    from core_utils import next_doc_number
    return await next_doc_number("purchase_returns", "debit_note_number", "DN-", width=5, scheme="shared")
...
async def create_purchase_return(payload, created_by: str) -> Dict[str, Any]:
    ...
    for it in payload.items:
        ...
        if roll_ids:
            ...
            if price <= 0:
                ...
                # dikonsumsi), bukan rata-rata polos unit_cost roll.
                _pairs = [(float(r.get("unit_cost") or r.get("base_unit_cost") or 0),
                           float(r.get("length_remaining", 0) or 0)) for r in sel]
                _pairs = [(c, w) for c, w in _pairs if c > 0 and w > 0]
                _tw = sum(w for _, w in _pairs)
                price = round(sum(c * w for c, w in _pairs) / _tw, 2) if _tw > 0 else (
                    _po_item_price(po, it.product_id) or float(prod.get("price", 0)))
    ...
    now = now_iso()
    # tarif efektif PO = ppn_amount ÷ dasar (net_subtotal/dpp) → sudah memuat DPP nilai lain
    _po_ppn = float((po or {}).get("ppn_amount", 0) or 0)
    _po_base = float((po or {}).get("net_subtotal") or (po or {}).get("dpp") or (po or {}).get("total_amount") or 0)
    _ppn_rate = round(_po_ppn / _po_base * 100.0, 4) if (_po_ppn > 0 and _po_base > 0) else 0.0
    ...
    doc = {
        ...
        # KN-B21 — PPN Masukan atas barang yang dikembalikan WAJIB dibalik: tarif
        # mengikuti PO asal (0 bila PO non-PPN / tidak ada PO).
        "ppn_rate": _ppn_rate, "ppn_amount": round(total * _ppn_rate / 100.0, 2),
        "grand_total": round(total + total * _ppn_rate / 100.0, 2),
...
async def _finalize_supplier_return_financials(ret: Dict[str, Any], actor: str, notes: str,
    ...
    if ret.get("po_id") and not is_refund:
        await db.purchase_orders.update_one(
            {"id": ret["po_id"]},
            # KN-B20 — basis hutang PO termasuk PPN (grand_total), maka nilai retur yang
            # dikurangkan juga termasuk PPN-nya; dulu PO yang diretur penuh menyisakan
            # hutang sebesar PPN selamanya.
            {"$inc": {"returned_amount": float(ret.get("grand_total") or
                                               (float(ret.get("total_amount", 0) or 0)
                                                + float(ret.get("ppn_amount", 0) or 0)))},
             "$set": {"updated_at": now}})
...
async def goods_back(return_id: str, actor: str, regrade: List[Dict[str, Any]] = None,
    ...
    for item in ret.get("items", []):
        for rid in (item.get("roll_ids") or []):
            ...
            # KN-B13 — gudang ASAL juga di-rebuild; dulu hanya tujuan → kuantitas karantina
            # hantu tertinggal selamanya di gudang asal.
            if roll.get("warehouse_id") and roll.get("warehouse_id") != dest_wh:
                segments.add((roll.get("product_id"), roll.get("warehouse_id"), roll.get("owner_entity_id")))
    for (pid, wid, eid) in segments:
        if pid and wid and eid:
            await rebuild_balance(pid, wid, eid)
