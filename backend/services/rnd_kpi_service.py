...
# Nilai versi desain berskala 0–2; dikali 50 agar sebanding dengan skor round 0–100.
DESIGN_SCORE_SCALE = 50.0
...
async def weights(entity_id: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "design": float(pol.get("kpi_weight_design") or 0),
...
def compute_grade(row: Dict[str, Any], w: Dict[str, Any]) -> Dict[str, Any]:
    ...
    design_pts = None
    if row.get("design_avg_score") is not None:
        design_pts = round(min(float(row["design_avg_score"]) * DESIGN_SCORE_SCALE, 100.0), 1)
        comps.append((float(w.get("design") or 0), design_pts))
    wsum = sum(weight for weight, _ in comps if weight > 0)
    if wsum <= 0:
        return {"grade_score": None, "grade_base": None, "grade_penalty": 0.0,
                "grade_design_pts": design_pts,
    ...
    return {"grade_score": score, "grade_base": round(base, 1),
            "grade_penalty": round(penalty, 1), "grade_design_pts": design_pts,
            **{f"grade_{k}": v for k, v in grade_of(score).items()}}


# ─── Design Studio — nilai versi & revisi per desainer ───────────────────────
def _version_date(v: Dict[str, Any]) -> Optional[date]:
    """Tanggal acuan versi desain: saat dinilai, kalau belum → saat diunggah."""
    return _as_date(v.get("score_at")) or _as_date(v.get("at"))
...
async def design_studio_stats(query: Optional[Dict[str, Any]], start: Optional[date]) -> Dict[str, Dict[str, Any]]:
    ...
    for d in rows:
        ...
        versions = [v for v in (d.get("versions") or [])
                    if not start or ((_version_date(v) or date.min) >= start)]
...
async def designer_kpi(query: Optional[Dict[str, Any]] = None, *, period: str = "all",
    ...
    # dan (sesuai bobot `rnd.kpi_weight_design`) ikut membentuk grade komposit.
    ...
    for name, ds in dstats.items():
        if _norm(name) not in known:
            row = {**_blank(ds["designer"]), "samples": 0, "rework": 0, "late_total": 0,
...
async def designer_kpi_trend(query: Optional[Dict[str, Any]] = None, *, months: int = 6,
    ...
    if metric in ("design", "design_score"):
        return await design_score_trend(query, months=months)
    metric = "avg_score" if metric in ("avg_score", "score") else "grade"
    today = date.today()
    keys = _last_months(months, today)
    keyset = set(keys)
    w = await weights(entity_id)
    dbuckets = await _design_month_buckets(query, keyset)
    ...
    for who in designers:
        ...
        for k in keys:
            ...
            row = {
                ...
                "design_avg_score": (round(dsg["sum"] / dsg["n"], 2) if dsg and dsg["n"] else None),
            }
...
# ═════════════════════════════════════════════════════════════════════════════
#  TREN NILAI DESAIN PER BULAN (Design Studio) — versi desain yang DINILAI
#  Tanggal acuan = saat versi dinilai (`score_at`), fallback saat diunggah (`at`).
# ═════════════════════════════════════════════════════════════════════════════
async def _design_month_buckets(query: Optional[Dict[str, Any]], keyset: set) -> Dict[str, Dict[str, Dict[str, Any]]]:
    """buckets[month][designer] = {sum, n, versions, acc} dari versi desain yang dinilai."""
    scope: Dict[str, Any] = {}
    if query and query.get("entity_id") is not None:
        scope["entity_id"] = query["entity_id"]
    rows = await db.design_gallery.find(scope, {"_id": 0, "created_by": 1, "versions": 1}).to_list(5000)
    buckets: Dict[str, Dict[str, Dict[str, Any]]] = {}
    for d in rows:
        who = str(d.get("created_by") or "").strip()
        if not who:
            continue
        for v in (d.get("versions") or []):
            ...
            if mk not in keyset:
                continue
            b = buckets.setdefault(mk, {}).setdefault(who, {"sum": 0.0, "n": 0, "versions": 0, "acc": 0})
            b["versions"] += 1
            if v.get("score") is not None:
                b["sum"] += float(v["score"])
                b["n"] += 1
...
async def design_score_trend(query: Optional[Dict[str, Any]] = None, *, months: int = 6) -> Dict[str, Any]:
    """Tren rata-rata nilai versi desain (0–2) per desainer per bulan untuk grafik."""
    keys = _last_months(months, date.today())
    buckets = await _design_month_buckets(query, set(keys))
    designers: set = set()
    for mk in keys:
        designers.update(buckets.get(mk, {}).keys())
    ...
    for who in designers:
        ...
        total_versions = 0
        for k in keys:
            b = buckets.get(k, {}).get(who)
            if not b:
                points.append({"month": k, "score": None, "versions": 0, "acc": 0})
                continue
            score = round(b["sum"] / b["n"], 2) if b["n"] else None
            total_versions += int(b["versions"])
            points.append({"month": k, "score": score, "versions": int(b["versions"]), "acc": int(b["acc"])})
            if score is not None:
                vals.append(score)
        if not vals:
            continue
        series.append({"designer": who, "points": points, "versions": total_versions,
    ...
    return {
        ...
        "month_labels": [_month_label(k) for k in keys],
        "metric": "design_score",
        "metric_label": "Nilai versi desain (0–2)",
        "scale_max": 2,
        "count": len(series),
        "designers": [s["designer"] for s in series],
        "series": series,
