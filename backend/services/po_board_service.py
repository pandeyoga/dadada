...
def receipt_facts(po: Dict[str, Any], tasks: Sequence[Dict[str, Any]],
    ...
    return {
        ...
        "receipt_variance": receipt_variance_summary(po.get("receipt_variances")),
    }


def receipt_variance_summary(variances: Any) -> Optional[Dict[str, Any]]:
    """FASE SL — ringkasan catatan selisih penerimaan (ditulis gudang saat GR selesai)."""
    lst = [v for v in (variances or []) if isinstance(v, dict)]
    if not lst:
        return None
    diff_qty = round(sum(float(v.get("diff_qty") or 0) for v in lst), 3)
    flagged = sum(len(v.get("label_variance_rolls") or []) for v in lst)
    unit = str(lst[-1].get("unit") or "")
    parts: List[str] = []
    if abs(diff_qty) > 1e-6:
        parts.append(f"{'+' if diff_qty > 0 else '−'}{abs(diff_qty):g} {unit}".strip())
    if flagged:
        parts.append(f"{flagged} roll label≠aktual")
    return {"count": len(lst), "diff_qty": diff_qty, "flagged_rolls": flagged,
            "label": " · ".join(parts) or "ada selisih",
            "notes": [str(v.get("note") or "") for v in lst if v.get("note")]}
