...
COLUMNS: Tuple[Tuple[str, str, float, float], ...] = (
    ...
    ("design_versions", "Versi desain", 20, 12),
    ("design_avg_score", "Nilai desain (0-2)", 24, 14),
    ("design_acc", "ACC desain", 18, 11),
    ("design_revisions", "Revisi desain", 20, 12),
...
def formula_of(rep: Dict[str, Any]) -> str:
    ...
    return (f"Nilai = tepat waktu {w.get('on_time', 0)}% + skor penilaian "
            f"{w.get('score', 0)}% + sekali-jadi (ACC) {w.get('acc', 0)}% + nilai desain "
            f"{w.get('design', 0)}% (0-2 dikali 50), dikurangi "
...
def designer_report_pdf(mine: Dict[str, Any], entity_name: str = "Kain Nusantara",
    ...
    metrics = [
        ...
        ("Versi desain (Design Studio)", _num(me.get("design_versions"))),
        ("Nilai desain rata (0–2)", _num(me.get("design_avg_score"))),
        ("Versi desain ACC", _num(me.get("design_acc"))),
        ("Revisi desain diminta", _num(me.get("design_revisions"))),
