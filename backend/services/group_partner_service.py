...
async def _next_supplier_code(taken: set) -> str:
    """KN-D15 — SATU generator kode supplier (atomik, sama dengan routers/suppliers.py);
    dulu dua aturan berbeda dan versi router tidak memfilter pola `SUP-` → kode kembar."""
    from core_utils import next_doc_number
    while True:
        code = await next_doc_number("suppliers", "code", "SUP-", scheme="shared")
