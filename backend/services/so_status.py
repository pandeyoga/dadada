...
def derive_stage_substatus(order: Dict[str, Any]) -> Tuple[str, List[str]]:
    ...
    # KN-D14 — baris backorder yang sudah `fulfilled` tidak dihapus dari daftar, jadi
    # daftar tidak kosong BUKAN bukti masih menunggu stok: hitung yang sisa qty-nya > 0.
    has_bo = bool(o.get("has_backorder")) or any(
        float(b.get("backorder_qty", 0) or 0) > 0.001 and b.get("status") != "fulfilled"
        for b in (o.get("backorders") or []))
