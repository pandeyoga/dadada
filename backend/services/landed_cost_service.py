...
async def next_voucher_number() -> str:
    ...
    return await next_doc_number("landed_cost_vouchers", "voucher_number", "LCV-", width=5, scheme="shared")
...
async def apply_allocation_to_rolls(voucher_number: str, allocations: List[Dict[str, Any]]) -> int:
    ...
    for a in allocations:
        ...
        # KN-B17 — roll yang sudah DIPECAH sebelum voucher disetujui: potongannya
        # (parent_roll_id/root_roll_id = roll asal) ikut menerima biaya per unit yang sama,
        # supaya HPP potongan tidak tertinggal di harga bruto.
        await db.inventory_rolls.update_many(
            {"$or": [{"parent_roll_id": a["roll_id"]}, {"root_roll_id": a["roll_id"]}],
             "id": {"$ne": a["roll_id"]},
             "landed_cost_refs": {"$ne": voucher_number}},
            {"$inc": {"unit_cost": a["per_unit"]},
             "$set": {"updated_at": now_iso()},
             "$push": {"landed_cost_refs": voucher_number}})
