...
async def summarize_for(user_id: Optional[str], role: str, day: str,
    ...
    if role != "admin":
        # PERAN & HAK AKSES — audiens + relevansi layar mengikuti pengaturan peran.
        from services.notification_scope import relevance_filter
        user_doc = (await db.users.find_one({"id": user_id}, {"_id": 0, "id": 1, "role": 1})
                    if user_id else None) or {"id": user_id, "role": role}
        q = {"$and": [q, await relevance_filter(user_doc)]}
