...
async def transition_special_order_status(
    ...
    # KN-B10 — CAS berprasyarat status saat ini: dua transisi bersamaan → tepat satu lolos.
    result = await db.special_orders.find_one_and_update(
        {"id": special_order_id, "status": current_status},
        {
            "$set": {
                "status": new_status,
                "updated_at": now_iso(),
    ...
    if not result:
        raise HTTPException(status_code=409,
                            detail=f"Status special order berubah dari '{current_status}' oleh proses lain. Muat ulang.")
    result.pop("_id", None)
    return result
