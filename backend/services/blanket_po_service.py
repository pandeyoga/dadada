...
async def create_blanket(payload, actor: Dict[str, Any]) -> Dict[str, Any]:
    ...
    products = {p["id"]: p for p in await db.products.find(
        {"id": {"$in": [it.product_id for it in payload.items]}}, {"_id": 0}).to_list(len(payload.items))}
    # R-7 (FASE F / PS-12) — gerbang produk sah SAMA dengan PO standar: SKU R&D yang belum
    # dirilis tidak boleh diikat kontrak blanket.
    ...
    await rnd_gate.assert_orderable(
        ...
        entity_id=payload.entity_id or "", where="Blanket PO")
