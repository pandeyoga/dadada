...
def approval_mode_of(rule: dict) -> str:
    m = rule.get("approval_mode")
    if m in ("global", "always", "never"):
        return m
    return "always" if rule.get("requires_approval") else "global"


APPROVAL_MODE_LABEL = {"global": "ikut ambang organisasi", "always": "selalu perlu persetujuan",
                       "never": "tanpa persetujuan"}
...
def _line(kind: str, rule: dict, gross: int, bases: dict = None) -> dict:
    ...
    return {
        ...
        "approval_mode": mode, "requires_approval": mode == "always",
...
async def listing(kind: str, org: str = ORG_ID, active: bool = None) -> list:
    ...
    for r in rows:
        ...
        r["approval_mode"] = approval_mode_of(r)
        r["requires_approval"] = r["approval_mode"] == "always"
        if kind == "coupon":
...
async def create(kind: str, payload: dict, actor: str, org: str = ORG_ID) -> dict:
    ...
    doc["approval_mode"] = approval_mode_of(doc)
    doc["requires_approval"] = doc["approval_mode"] == "always"
...
async def update(kind: str, rule_id: str, patch: dict, actor: str, org: str = ORG_ID) -> dict:
    ...
    if "approval_mode" in patch or "requires_approval" in patch:
        mode = patch.get("approval_mode") or ("always" if patch.get("requires_approval") else "never")
        patch["approval_mode"] = mode
        patch["requires_approval"] = mode == "always"
...
async def compute_discounts(org: str, *, unit: dict, gross: int, discount_scheme_id: str = None,
    ...
    thresholds = await approval_thresholds(org)
    approval = evaluate_approval(lines, gross, **thresholds)
    return {"lines": lines, "total": int(price_total), "by_target": by_target,
            "needs_approval": approval["needs_approval"],
            "approval_reasons": approval["reasons"], "approval_thresholds": thresholds,
            "coupon_code": (coupon_code or "").strip().upper() or None}


async def approval_thresholds(org: str) -> dict:
    """Ambang GLOBAL persetujuan manajer (Pusat Konfigurasi › Penawaran)."""
    return {"max_pct": float(await cfg.get("quotation.discount_max_pct_sales", org_id=org) or 0),
            "min_amount": int(await cfg.get("pricing.approval_min_amount", org_id=org) or 0)}


def evaluate_approval(lines: list, gross: int, *, max_pct: float = 0, min_amount: int = 0) -> dict:
    """SATU aturan persetujuan untuk penawaran DAN reservasi/SPR.

    Per baris: `always` → wajib; `never` → dikecualikan dari ambang; `global` → dihitung
    bersama terhadap ambang persen (potongan harga/DP/BF ÷ harga) dan ambang nominal (semua
    sasaran, termasuk komponen biaya all-in)."""
    reasons = []
    for x in lines:
        if x.get("approval_mode", "always" if x.get("requires_approval") else "global") == "always":
            reasons.append({"code": "rule_always", "rule": x.get("code"),
                            "label": f"{x.get('source_label')} {x.get('code')} ditandai selalu perlu persetujuan."})
    glob = [x for x in lines if x.get("approval_mode", "global") == "global"]
    price_part = sum(int(x.get("amount") or 0) for x in glob if x.get("target") in ("price", "dp", "booking_fee"))
    total_part = sum(int(x.get("amount") or 0) for x in glob)
    pct = round(price_part / gross * 100, 2) if gross else 0
    if price_part and max_pct is not None and pct > float(max_pct):
        reasons.append({"code": "over_pct", "pct": pct, "limit": float(max_pct),
                        "label": f"Potongan harga {pct:g}% melewati batas sales {float(max_pct):g}%."})
    if min_amount and total_part >= int(min_amount):
        reasons.append({"code": "over_amount", "amount": total_part, "limit": int(min_amount),
                        "label": (f"Total potongan Rp {total_part:,} mencapai ambang nominal "
                                  f"Rp {int(min_amount):,}.").replace(",", ".")})
    return {"needs_approval": bool(reasons), "reasons": reasons, "pct": pct, "amount": total_part}
...
async def options_for_unit(org: str, unit: dict, lead_id: str = None) -> dict:
    ...
    for kind in ("discount_scheme", "promo"):
        ...
        out[kind + "s"] = [
            {**r, "target": r.get("target") or "price", "target_label": target_label(r),
             "approval_mode": approval_mode_of(r),
             "requires_approval": approval_mode_of(r) == "always",
    ...
    out["approval_thresholds"] = await approval_thresholds(org)
    return out
...
async def seed_defaults(org: str = ORG_ID) -> dict:
    ...
    for kind, doc in demo:
        ...
        if await coll.find_one({"org_id": org, "code": doc["code"]}, {"_id": 1}):
            continue
        mode = "always" if doc.get("requires_approval") else "global"
