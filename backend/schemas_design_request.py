...
class DesignRequestCreate(BaseModel):
    ...
    customer_id: str = ""
    line_code: str = ""
    target_type: str = "motif"        # LEGACY (motif | pattern | artwork) — layar kini memakai kategori
    #: Sinkron dengan Design Studio: Kategori Pattern + Kategori Design (master yang sama).
    category_code: str = ""
...
class DesignRequestCreateDesign(BaseModel):
    """Desainer membuat desain di Design Studio LANGSUNG dari permintaan (tertaut otomatis)."""
    title: str = ""


class DesignRequestLinkDesign(BaseModel):
    """Menautkan desain yang sudah ada di Design Studio ke permintaan ini."""
    gallery_id: str
