...
async def simulate(org: str = ORG_ID, *, unit_id: str, addons: list = None,
    ...
    if allin_scheme_id:
        ...
        costs = await ae.resolve_scheme(org, allin_scheme_id, int(net), unit.get("project_id"),
                                        ...
                                        booking_fee=max(0, bases["booking_fee"] - by_target["booking_fee"]))
...
async def create(org: str = ORG_ID, *, lead_id: str, unit_id: str, addons: list = None,
                 ...
                 discount_scheme_id: str = None, promo_id: str = None,
                 coupon_code: str = None, allin_scheme_id: str = None,
                 booking_fee: int = None) -> dict:
    """Simpan penawaran (versi 1 atau revisi). Diskon di atas kewenangan → minta persetujuan.
    Skema all-in & booking fee ikut disimpan (form penawaran ≡ form reservasi) sehingga
    komponen pembayaran yang dijanjikan sama dengan yang dibawa ke reservasi."""
    ...
    calc = await simulate(org, unit_id=unit_id, addons=addons, scheme_id=scheme_id,
                          ...
                          allin_scheme_id=allin_scheme_id, booking_fee=booking_fee)
    pending = [x for x in calc.get("discount_lines") or [] if x.get("pending")]
    if pending:
        raise ValueError("Potongan " + ", ".join(x["code"] for x in pending)
                         + " menyasar komponen biaya yang tidak ada pada transaksi ini — pilih skema "
                         "all-in yang memuat komponennya, atau pilih promo lain.")
    ...
    doc = {
        ...
        # Komponen pembayaran lengkap (identik dengan reservasi): biaya all-in, booking fee, total.
        "allin_scheme_id": allin_scheme_id or None, "costs": calc.get("costs"),
        **{k: calc.get(k) for k in ("by_target", "bases", "booking_fee_gross", "booking_fee_net",
                                    "booking_fee", "cost_discount_amount", "addon_discount_amount",
                                    "total_discount_amount", "buyer_total", "payment_breakdown",
                                    "terms_total", "as_of")},
...
async def convert(org: str, quotation_id: str, actor: str, note: str = None) -> dict:
    ...
    if (q.get("costs") or {}).get("components") is not None:
        extra["costs"] = dict(q["costs"])  # skema all-in penawaran dibawa ke deal → kontrak
    deal = await sr.reserve(org, lead=lead, unit=unit, actor=actor, notes=note,
                            booking_fee=int(booking_fee or 0), extra=extra)
    if q.get("coupon_code"):
