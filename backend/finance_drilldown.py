...
async def drilldown(org: str, key: str, bucket: str = None) -> dict:
    ...
    if key == "ar_outstanding":
        ...
        title, href_all = "Utang (AP) disetujui belum dibayar", AP_BASE
        for b in await db.ap_invoices.find({"org_id": org, "status": {"$in": ["approved", "partial"]}}, {"_id": 0}).to_list(2000):
            rows.append(_ap_row(b, int(b.get("net", 0)) - int(b.get("paid", 0))))
    elif key == "ap_pending":
