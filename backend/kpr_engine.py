...
async def kpr_advance(org: str, contract_id: str, stage: str, payload: dict,
    ...
    if stage == "sp3k":
        ...
        harga = int(bd_sp3k.get("nett_price") or bd_sp3k.get("net_price") or bd_sp3k.get("gross_price") or 0)
        ...
        # Plafon ≠ porsi bank → AR disesuaikan (selisih kurang = termin pembeli; lebih = porsi bank naik).
        import kpr_plafon as kp
        inv_pv = await db.ar_invoices.find_one({"org_id": org, "deal_id": c["deal_id"]}, {"_id": 0, "items": 1})
        pv = kp.preview(inv_pv, int(payload["plafon"])) if inv_pv else None
        if pv and pv["kind"] == "shortfall" and not payload.get("shortfall_due_date"):
            raise ValueError(f"Plafon {_rp(payload['plafon'])} lebih kecil dari porsi bank {_rp(pv['bank_portion'])} — "
                             f"selisih {_rp(pv['shortfall'])} menjadi kewajiban pembeli. Isi JATUH TEMPO selisih plafon.")
        if pv and pv["kind"] == "excess" and not pv["excess_ok"]:
            raise ValueError(f"Plafon melebihi porsi bank {_rp(pv['bank_portion'])} sebesar {_rp(pv['excess'])}, "
                             f"tetapi termin pembeli yang bisa dialihkan hanya {_rp(pv['buyer_reducible'])}.")
    ...
    if stage == "sp3k":
        ...
        summary = await kp.reconcile(org, c, fresh, int(payload["plafon"]),
                                     payload.get("shortfall_due_date"), actor)
        await db.financing_apps.update_one({"id": app["id"]}, {"$set": {"plafon_reconcile": summary}})
        try:
