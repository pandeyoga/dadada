...
## 3ac) Sesi 2026-09-11 — Pencairan KPR kartu pelanggan ≡ skema pencairan bank (SELESAI)

| Bagian | Status |
|---|---|
| Pemulihan lingkungan dari repo `bavafatata/sipro` (deps, `.env` backend + JWT_SECRET/SEED_DEMO_USERS, `memory/test_credentials.md`) | **SELESAI** |
| `POST /financing/{fid}/disburse` (tombol **Cairkan** kartu KPR pelanggan) kini mendelegasikan ke `kpr_disburse.disburse`: tahap dari skema, nominal bebas ditolak 400, kuitansi `kpr` + jurnal, `ar_booking` memuat `receipt_no`/`tranche`; `DisbursementCreate` menerima `tranche_code`/`date`/`file_id` (milestone opsional) | **SELESAI** |
| `kpr_disburse`: `bank_compatible` (fuzzy: 'BTN' ↔ 'BTN KC Bandung'), `schemes_for_bank`, `default_scheme` (1 skema khusus bank → otomatis; >1 → pilih manual; lalu default code / satu-satunya 'semua bank'), `ensure_scheme` pakai `approved_plafon or plafon`; `build_tranches`: tahap bernominal = retensi tetap, tahap ber-% membagi SISA plafon, semua-nominal ≠ plafon → T1 menyerap selisih; `validate_scheme` hanya menuntut Σ% = 100 untuk tahap ber-% | **SELESAI** |
| `GET /kpr-disbursement-schemes?bank=` → skema cocok + `default_id` | **SELESAI** |
| SP3K: validasi plafon memakai `nett_price` (dulu `net_price` — tak pernah cocok) | **SELESAI** |
| UI `DisburseDialog` ditulis ulang: blocker (kontrak/akad), pemilih skema bila tahapan kosong, daftar tahap + status/syarat, select tahap (syarat belum terpenuhi disabled), nominal terisi otomatis (edit hanya `finance:manage`), tanggal, rekening, catatan. `CustomerFinancingTab`: skema + tahapan + kuitansi per pencairan. `AddFinancingDialog`: hint skema pencairan bank (`financing-disb-scheme-hint`) | **SELESAI** |
