...
if [ $AUTH_READY -eq 1 ]; then
  ...
  # AUDIT 2026-09-21 (KN-A01..A07, A08, C03, C05) — penjaga entitas jalur TULIS + isolasi
  # pesanan sampel (R-3a; menggantikan uji lama yang menunjuk /sample-requests yang mati)
  # + konstanta bersama (GREEN gate RFID · status fisik roll · PO terbuka). Masuk gate
  # DEFAULT supaya regresi kelas "penjaga dipasang di GET lalu terlewat di aksi" memerah.
  run_gate "audit_2026_09_21 (penjaga entitas aksi tulis · R-3a sampel · konstanta bersama)" \
    "cd backend && python -m pytest tests/test_kn_audit_2026_09_21.py -q"
