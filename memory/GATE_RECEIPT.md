...
- **Waktu:** 2026-09-21 09:51:39
- **Mode:** `ci`  ·  **Durasi total:** 227s  ·  **Pekerja statik:** 2
- **Backend:** RUNNING + auth siap (gate runtime dijalankan)

| Gate | Hasil |
|------|-------|
| guard:auth_coverage (INV-AUTH-01) | FAIL (1s) |
| guard:auth_coverage SELF-TEST (bukti-merah penjaga auth) | PASS (0s) |
| validate_compliance (file/naming/docs/api/env) | FAIL (1s) |
...
| guard:escape_layers (INV-UI-10, Esc menutup lapisan teratas saja) | FAIL (2s) |
...
| guard:role_label (INV-ROLE-01, peran dari registry & izin) | FAIL (1s) |
...
| guard:warehouse_scope SELF-TEST (E4.1, gudang khusus badan usaha) | FAIL (2s) |
...
| verify_entity_scoping (F0-C, DB + STATIK anti-kebocoran PT) | FAIL (1s) |
...
| audit_entity_isolation (E0.9/E0.10 — 0 kebocoran lintas-entitas) | FAIL (11s) |
| POC FASE E-0 (bukti-merah L1–L21: notifikasi·denda·audit·lot·AR·transfer·dokumen·pratinjau) | FAIL (2s) |
| POC FASE E-3 (mode “Semua Entitas” hanya-lihat: 409 menuntun · master bersama tetap boleh) | FAIL (2s) |
...
| POC FASE E-5 (papan stok agregat · pegging · mutasi lintas-PT nama singkat · kartu riwayat) | FAIL (1s) |
...
| POC FASE E-8 G1 (peran sales_admin & finance · pemisahan tugas · penugasan entitas) | FAIL (4s) |
| POC FASE E-8 G2/G3 (meja admin sales & finance · verifikasi · keputusan pemenuhan) | FAIL (4s) |
| POC FASE E-9 (rantai jual→beli internal antar-PT→retur berantai · 41 pemeriksaan) | PASS (5s) |
...
| audit_sales_roles_ux (SEMUA peran: nol layar & panel mati) | FAIL (53s) |
...
| POC FASE F-6 (pensiun mesin generik · 14 antrean nyata · anti dobel-hitung) | FAIL (3s) |
...
| guard:approval_queues (INV-APPR-01, tiap pintu keputusan punya antrean) | FAIL (0s) |
...
| guard:uom_vocab SELF-TEST (bukti-merah kosakata satuan, 23 kasus dua arah) | FAIL (1s) |
| guard:uom_vocab (INV-UOM-02, satuan dokumen ⊆ master uoms · alias tak kembar · pemilih satuan dari master) | FAIL (2s) |
...
| POC FASE U (dua satuan: PO→terima→PDF/CSV · retur turun serentak · satuan lini · dokumen lama "—") | FAIL (10s) |
...
| POC FASE P (papan PO per lini: tahap dari master · sales dirunut · inspect turunan · terima dihitung · nol residu) | FAIL (1s) |
| POC FASE D · PERMINTAAN DESAIN (DSR) (tugas→serah→revisi ber-alasan→ACC · rapor = hitung-ulang · peran ke-7 sempit tapi jujur) | FAIL (5s) |
...
| POC FASE I (inspeksi & QC: SPK otomatis · grade dari mesin lama · tahanan warna dilepas manajer · nol residu) | FAIL (10s) |
| POC FASE N (notifikasi beralamat: izin & divisi · dedupe per orang · nol siaran 'all') | PASS (1s) |
| audit_endpoint_sweep (semua GET → 5xx · paralel) | PASS (6s) |
