...
const server = http.createServer((req, res) => {
  ...
  fs.stat(target, (err, stat) => {
    ...
    // Aset ber-hash yang hilang (build lama) → 404, jangan kirim index.html sebagai JS/CSS
    if (/^\/static\//.test(req.url || "") || /\.(js|css|map|png|jpg|svg|woff2?)$/.test((req.url || "").split("?")[0])) {
