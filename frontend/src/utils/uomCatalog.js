...
let _factorByWord = {};  // KN-C06 — kata satuan (code/alias, lowercase) → factor_to_base dari MASTER server

/** Diisi `useUomConversions` setiap kali katalog server termuat/di-reload. */
export function setUomCatalogUnits(units, unitsMaster) {
  _units = Array.isArray(units) ? units : [];
  _factorByWord = {};
  for (const u of [...(Array.isArray(unitsMaster) ? unitsMaster : []), ..._units]) {
    const f = Number(u?.factor_to_base);
    if (!(f > 0)) continue;
    for (const w of [u.code, ...(u.aliases || [])]) {
      if (w) _factorByWord[String(w).toLowerCase()] = f;
    }
  }
}

/** KN-C06 — faktor ke satuan dasar dimensi menurut MASTER server (null bila tak dikenal). */
export function uomCatalogFactor(word) {
  const f = _factorByWord[String(word || "").toLowerCase()];
  return f > 0 ? f : null;
}
