...
import { uomCatalogUnits, uomDimensionOf, uomLabelOf, uomCatalogFactor } from "./uomCatalog";
...
export function convFactor(product, unit) {
  ...
  // KN-C06 — satuan yang ditambahkan pemilik lewat Master Data (mis. "Rol" = 50 m) dikenali
  // lewat faktor MASTER server, bukan jatuh diam-diam ke 1×.
  const cu = uomCatalogFactor(u), cb = uomCatalogFactor(base);
  if (cu != null && cb != null && uomDimensionOf(u) && uomDimensionOf(u) === uomDimensionOf(base)) return cu / cb;
