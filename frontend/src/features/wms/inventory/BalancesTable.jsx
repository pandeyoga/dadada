              ...
              return (
                <tr key={b.id}
                  ...
                  <td className="px-3 py-2">
                    ...
                    {/* FASE SL — alias supplier (kode & nama versi pabrik) */}
                    {(b.supplier_codes || []).length > 0 && (
                      <p data-testid={`balance-supplier-alias-${b.id}`} className="text-[9.5px] text-[#0058CC]">
                        pabrik: {b.supplier_codes.map((c) => `${c.supplier_sku}${c.supplier_item_name ? ` — ${c.supplier_item_name}` : ""}`).join(" · ")}
                      </p>
                    )}
                  </td>
                  <td className="px-3 py-2">
