...
export default function RndSpecsView({ currentUser, selectedEntity, focus, onFocusConsumed }) {
  ...
  const canCreate = (currentUser?.permissions?.rnd || []).includes('create');
  const canManage = (currentUser?.permissions?.rnd || []).includes('manage');
  ...
  useEffect(() => {
    ...
    if (focus.create) {
      if (canCreate && canWrite) { setInitialCatalog(focus); setShowForm(true); }
      else setError(canWrite ? "Anda tidak berwenang membuat spesifikasi." : writeBlockHint);
    }
    onFocusConsumed?.();
  ...
  return (
    <div data-testid="rnd-specs-view">
          ...
          <div className="flex items-center gap-2">
            ...
            {canCreate && (
              <button className="primary-button" data-testid="rnd-spec-create-button"
                disabled={!canWrite} title={writeBlockHint || undefined}
                onClick={() => { setInitialCatalog(null); setShowForm(true); }}>
      ...
      <div className="section-card overflow-x-auto">
        <div className="min-w-[850px] grid grid-cols-[130px_1.5fr_1fr_130px_150px_120px] px-3 py-1.5 bg-[#FAFBFC] text-[10px] font-bold uppercase text-[#6B6B73] border-b border-[#EFF0F2]">
            ...
            {filtered.map((s) => {
              ...
              return (
                <div key={s.id} data-testid={`rnd-spec-row-${s.id}`}
                  className="min-w-[850px] grid grid-cols-[130px_1.5fr_1fr_130px_150px_120px] items-center px-3 py-2.5 hover:bg-[#FAFBFC]">
      ...
      {showForm && canCreate && canWrite && (
        <SpecFormModal selectedEntity={selectedEntity} types={types} initial={initialCatalog}
