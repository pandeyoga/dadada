...
import ProductColorChip, { productColorText } from "../../../components/ProductColorChip";
          ...
          <div className="grid grid-cols-[1fr_68px_130px_56px_88px_52px_130px_auto] gap-2">
            <KNSelect data-testid="item-product-select" value={newItem.product_id}
              ...
              options={[
                ...
                ...products.map(p => productOption(p, `${p.sku} - ${p.name}${supplierCodesLabel(p)}${productColorText(p, formData.supplier_id) ? ` · ${productColorText(p, formData.supplier_id)}` : ""}`)),
              ]}
            />
            ...
            {formData.items.map((item, i) => {
              ...
              return (
                <div key={i} data-testid={`po-item-row-${i}`}
                  ...
                  <span className="min-w-0 truncate">{p?.sku} — {p?.name}<ProductColorChip product={p} supplierId={formData.supplier_id} compact className="ml-1" /></span>
