                ...
                {rows.map((r) => (
                  <div key={r.po_id} data-testid={`po-board-row-${r.po_id}`}
                    ...
                    <div className="min-w-0">
                      ...
                      {r.receipt_variance && (
                        <span data-testid={`po-board-variance-${r.po_id}`}
                              title={(r.receipt_variance.notes || []).join("\n")}
                              className="mt-0.5 inline-block whitespace-nowrap rounded border border-amber-200 bg-amber-50 px-1.5 py-0.5 text-[9.5px] font-semibold text-amber-700">
                          Selisih {r.receipt_variance.label}
                        </span>
                      )}
                    </div>
