...
import { productColorText } from "../../components/ProductColorChip";
...
export default function RFQCreateModal({ open, selectedEntity, onClose, onCreated, onError }) {
  ...
  const productOptions = useMemo(() => products.map((p) => ({ value: p.id, label: `${p.sku} — ${p.name}${productColorText(p) ? ` · ${productColorText(p)}` : ""}` })), [products]);
