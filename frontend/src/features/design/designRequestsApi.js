...
/** Desain Studio yang masih bisa ditautkan (berjalan & belum punya permintaan). */
export const DSR_LINKABLE_DESIGN_STATUSES = ["draft", "revision", "pending_approval", "in_review"];
export async function linkableDesigns() {
  const r = await axios.get(`${API}/design-gallery`);
  const rows = Array.isArray(r.data) ? r.data : (r.data?.items || []);
  return rows.filter((g) => DSR_LINKABLE_DESIGN_STATUSES.includes(g.status || "draft") && !g.request_id);
}

/** Desainer membuat desain Studio langsung dari permintaan (tertaut otomatis → Dikerjakan). */
export async function createDesignFromRequest(id, title = "") {
  const r = await axios.post(`${API}/design-requests/${id}/create-design`, { title });
  return r.data || null;
}

export async function linkDesignToRequest(id, gallery_id) {
  const r = await axios.post(`${API}/design-requests/${id}/link-design`, { gallery_id });
  return r.data || null;
}

export async function updateDesignRequest(id, patch) {
  const r = await axios.patch(`${API}/design-requests/${id}`, patch);
  return r.data || null;
}

/** Gambar referensi brief — ikut otomatis ke tab Referensi desain yang dibuat/ditautkan. */
export const referenceUrl = (id, fileId) => `${API}/design-requests/${id}/references/${fileId}`;
export async function uploadReference(id, file, caption = "") {
  const fd = new FormData();
  fd.append("file", file);
  const r = await axios.post(`${API}/design-requests/${id}/references`, fd, { params: caption ? { caption } : {} });
  return r.data || null;
}
export async function deleteReference(id, fileId) {
  const r = await axios.delete(`${API}/design-requests/${id}/references/${fileId}`);
  return r.data || null;
}

/** Urutan tahapan permintaan untuk stepper (revisi = cabang balik ke desainer). */
export const DSR_STEPS = [
