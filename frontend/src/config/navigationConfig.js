  ...
  },
  {
    ...
    items: [
      { id: "config-center", label: "Pusat Konfigurasi", icon: SlidersHorizontal,
        path: "/config", roles: ADMIN_SIDE, resource: "settings", action: "manage" },
...
export function buildNavGroups(role, can = null) {
  // Dengan `can` (izin efektif dari /auth/me): item ber-`resource` TAMPIL bila izinnya ada —
  // apa pun nama perannya. Dulu daftar peran statis (`roles`) tetap menjadi syarat, sehingga
  // izin yang diberikan admin di layar Hak Akses tidak pernah membuka menunya. Item tanpa
  // `resource` (Site Plan, Beranda) tetap mengikuti daftar peran; tanpa `can` (audit gate)
  // perilaku lama berbasis peran berlaku.
  const custom = !!can && !ALL.includes(role);
  const roleOk = (it) => it.roles.includes(role) || (custom && !!it.resource);
  const visible = (it) => {
    if (can && it.resource) return can(it.resource, it.action || "view");
    return roleOk(it) && (!can || !it.resource || can(it.resource, it.action || "view"));
  };
  const result = [];
  for (const entry of NAV_STRUCTURE) {
    if (!entry.roles.includes(role) && !custom && !can) continue;
