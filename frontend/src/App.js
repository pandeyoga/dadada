...
function RequireLegal({ children }) {
  const { can } = useAuth();
  if (!can("legal", "view")) return <Navigate to="/" replace />;
  return children;
}

// Pengguna ikut RBAC resource `users` (bisa dibuka ke peran lain dari layar Hak Akses).
function RequireUsers({ children }) {
  const { can } = useAuth();
  if (!can("users", "view")) return <Navigate to="/" replace />;
  return children;
}
    ...
    <AuthProvider>
      ...
      <BrowserRouter>
        <Routes>
          ...
          {/* Halaman legal PUBLIK (UU PDP + Meta App Review): privasi, S&K, penghapusan data. */}
          <Route path="/privacy" element={<LegalPublicPage />} />
          <Route path="/terms" element={<LegalPublicPage />} />
          <Route path="/data-deletion" element={<LegalPublicPage />} />
          <Route element={<RequireAuth><AppShell /></RequireAuth>}>
            ...
            <Route path="/admin/users" element={<RequireUsers><AdminUsers /></RequireUsers>} />
