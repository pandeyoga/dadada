...
export function ReferenceProvider({ children }) {
  ...
  // Nilai baru dari pemilih disimpan ke master organisasi (POST /reference/{group}/values)
  // supaya tetap ada walau form yang sedang diisi batal disimpan. Grup yang tidak
  // menerima nilai baru (400) jatuh ke nilai lokal saja.
  const addValue = useCallback(async (group, val) => {
    try {
      const res = await api.post(`/reference/${group}/values`, { value: val });
      const opts = res.data?.data?.options;
      if (opts) {
        setRegistry((prev) => ({ ...prev, [group]: { ...(prev[group] || {}), options: opts } }));
      }
      return res.data?.data?.value || val;
    } catch { return val; }
  }, []);
  ...
  const value = useMemo(() => {
    ...
    return { registry, maps, options, labelOf, groupMeta, loading, error, cachedAt,
      reload: load, addValue };
  }, [registry, maps, loading, error, cachedAt, load, addValue]);
...
export function useReference() {
  ...
  if (!ctx) {
    ...
    return {
      ...
      loading: false, error: "", cachedAt: null, reload: () => {}, addValue: async (_g, v) => v,
    };
  }
  return ctx;
}
