...
export default function ProductSelect({
  ...
  label = "Pilih produk…", triggerTestId = "product-select-trigger", disabled = false, supplierId = "",
}) {
  ...
  return (
    <>
      ...
      {open && createPortal(
        <ProductPickerModal stage={stage} selectedId={value} supplierId={supplierId}
...
function ProductPickerModal({ stage, selectedId, onClose, onPick, supplierId = "" }) {
           ...
           : filtered.map((p) => (
            <button key={p.id} data-testid={`product-select-item-${p.id}`} onClick={() => onPick(p)}
              ...
              <div className="min-w-0 flex-1">
                ...
                <ProductColorChip product={p} supplierId={supplierId} className="mt-0.5" />
              </div>
