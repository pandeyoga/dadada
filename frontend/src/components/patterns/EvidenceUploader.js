...
export default function EvidenceUploader({
  ...
  testId = "evidence-input", label = "Lampirkan bukti", accept = "*/*",
  names = {}, hint = null, required = false,
}) {
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-[11px] text-muted-foreground">
        {value.length}/{max} lampiran · {hint || `semua jenis berkas (gambar, PDF, dokumen) · maks ${MAX_MB}MB · disimpan utuh (tanpa kompresi) agar sah sebagai bukti.`}
      </p>
      {value.length ? (
        ...
        <div data-testid={required ? `${testId}-required-empty` : undefined}
          className={`flex items-center gap-1.5 rounded-md border border-dashed px-2.5 py-2 text-[11px] ${required ? "border-rose-300 bg-rose-50 text-rose-700" : "bg-secondary/30 text-muted-foreground"}`}>
          <Paperclip className="h-3.5 w-3.5" /> {required ? "Lampiran WAJIB — belum ada berkas dipilih." : "Belum ada lampiran dipilih."}
        </div>
      )}
    </div>
  );
