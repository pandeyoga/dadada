    ...
    <div data-testid={CUSTOMERS.financingSection} className="space-y-3">
      ...
      {financings.length === 0 ? (
        ...
        <div className="space-y-3">
          {financings.map((f) => {
            const plafon = f.approved_plafon || f.plafon || 0;
            const remaining = plafon - (f.disbursed_total || 0);
            return (
              <div key={f.id} data-testid={CUSTOMERS.financingRow}
                ...
                <div className="mt-1.5 grid gap-x-4 gap-y-1 text-xs text-muted-foreground sm:grid-cols-3">
                  <span>Plafon: <MoneyText value={plafon} className="font-medium text-foreground" /></span>
                ...
                {(f.tranches || []).length ? (
                  <div data-testid="financing-tranches" className="mt-2 border-t pt-2 text-xs">
                    <p className="text-muted-foreground">Skema pencairan: <b className="text-foreground">{f.disbursement_scheme_name}</b>
                      {f.disbursement_scheme_auto ? " · otomatis dari konfigurasi" : ""}</p>
                    {f.tranches.map((t) => (
                      <div key={t.code} className="flex justify-between py-0.5" data-tranche={t.code} data-status={t.status}>
                        <span className="text-muted-foreground">{t.code} · {t.name} · syarat {t.condition}</span>
                        <span className="flex items-center gap-2"><MoneyText value={t.amount} />
                          <span className={`rounded-full px-1.5 py-0.5 text-[10px] ${t.status === "dicairkan" ? "bg-emerald-100 text-emerald-800" : "bg-secondary"}`}>{t.status}</span></span>
                      </div>
                    ))}
                  </div>
                ) : null}
                {(f.disbursements || []).length > 0 ? (
                  <div className="mt-2 border-t pt-2 text-xs">
                    {f.disbursements.map((d) => (
                      <div key={d.id} className={`flex justify-between py-0.5 ${d.status === "dibatalkan" ? "line-through opacity-60" : ""}`}>
                        <span className="text-muted-foreground">
                          {d.tranche_name || d.milestone} · {formatDateWIB(d.created_at)}{d.receipt_no ? ` · kuitansi ${d.receipt_no}` : ""}
                        </span>
