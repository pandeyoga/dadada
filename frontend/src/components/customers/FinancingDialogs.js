...
export function DisburseDialog({ open, onOpenChange, financing, onDone }) {
  ...
  useEffect(() => {
    if (!open) return;
    setForm({ tranche_code: "", amount: "", date: "", note: "", cash_account_id: "", file_ids: [] });
    load();
  }, [open, financing]);   // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const submit = async () => {
    ...
    if (!(form.file_ids || []).length) { toast.error("Bukti pencairan bank wajib dilampirkan."); return; }
    setBusy(true);
    try {
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto">
        ...
        {!blocker && tranches.length ? (
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-1.5 sm:col-span-2"><Label>Bukti pencairan bank (wajib)</Label>
              <EvidenceUploader value={form.file_ids || []} onChange={(v) => set("file_ids", v)} max={1} required
                ownerType="receipt_proof" ownerId={financing?.deal_id} testId="financing-disburse-proof-input"
                label="Bukti pencairan" hint="nota kredit / mutasi rekening dari bank — semua jenis berkas." /></div>
          </div>
        ) : null}
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)} disabled={busy}>Batal</Button>
          <Button data-testid={CUSTOMERS.disburseSubmit} onClick={submit} disabled={busy || !!blocker || !form.tranche_code || !(form.file_ids || []).length}>
