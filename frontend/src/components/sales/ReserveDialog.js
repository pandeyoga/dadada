...
export default function ReserveDialog({
  ...
  const submit = async () => {
    ...
    try {
      ...
      const pa = res.data.data?.pricing_approval;
      toast.success(pa?.state === "pending"
        ? "Unit di-reserve — potongan MENUNGGU persetujuan manajer (booking & SPR tertahan sampai disetujui)."
        : "Unit berhasil di-reserve (hold aktif) — rincian harga tersimpan.");
      onOpenChange(false);
      ...
      <DialogContent data-testid={RESERVE.dialog} className="max-h-[94vh] overflow-y-auto sm:max-w-5xl">
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,7fr)_minmax(0,5fr)]">
          ...
          <div className="space-y-3 lg:sticky lg:top-0 lg:self-start">
            ...
            {calc?.needs_discount_approval ? (
              <div data-testid="reserve-approval-box" className="space-y-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
                <p className="font-medium">Potongan ini memerlukan persetujuan manajer</p>
                <ul className="list-disc pl-4 text-xs">
                  {(calc.approval_reasons || []).map((r, i) => <li key={i}>{r.label}</li>)}
                </ul>
                <p className="text-xs">
                  {mayOverrideZero
                    ? "Anda memegang izin persetujuan harga — reservasi langsung tercatat DISETUJUI atas nama Anda (alasan wajib)."
                    : "Reservasi tetap dibuat dan unit ditahan, tetapi booking & SPR tertahan sampai manajer menyetujui."}
                </p>
                <Textarea data-testid="reserve-discount-reason" rows={2} className="bg-background"
                  placeholder="Alasan / dasar potongan (min. 5 huruf)" value={form.discount_reason || ""}
        ...
        <DialogFooter>
          ...
          <Button data-testid={DEALS.reserveSubmit} onClick={submit}
            disabled={busy || (!!calc?.needs_discount_approval && (form.discount_reason || "").trim().length < 5)}>
            {busy ? "Memproses..." : calc?.needs_discount_approval && !mayOverrideZero ? "Reservasi (minta persetujuan)" : "Reservasi"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
