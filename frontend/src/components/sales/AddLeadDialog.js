...
import { CHUB, LEADS } from "@/constants/testIds";
...
export default function AddLeadDialog({ open, onOpenChange, onDone }) {
  ...
  const [form, setForm] = useState({ name: "", phone: "", email: "", source: "manual", interest_unit_type: "", notes: "", partner_id: "", assigned_to: "" });
  ...
  // PIC: sama seperti tangkapan WA — bisa dipilih langsung; kosong = round-robin otomatis.
  useEffect(() => {
    if (!open) return;
    api.get("/partners", { params: { status: "active", limit: 200 } })
      ...
      .catch(() => setPartners([]));
    if (canAssign) {
      api.get("/leads/assignees").then((r) => setAssignees(r.data.data || [])).catch(() => setAssignees([]));
    }
  }, [open, canAssign]);
  ...
  const submit = async () => {
    ...
    try {
      ...
      onOpenChange(false);
      setForm({ name: "", phone: "", email: "", source: "manual", interest_unit_type: "", notes: "", partner_id: "", assigned_to: "" });
      onDone && onDone();
    } catch (e) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          {canAssign ? (
            <div className="space-y-1.5 sm:col-span-2">
              <Label>PIC / Sales pemegang lead</Label>
              <Select value={form.assigned_to || AUTO} onValueChange={(v) => set("assigned_to", v === AUTO ? "" : v)}>
                <SelectTrigger data-testid={CHUB.leadPic} aria-label="PIC lead">
                  <SelectValue placeholder="Otomatis (round-robin)" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value={AUTO}>Otomatis (round-robin)</SelectItem>
                  {assignees.map((a) => (
                    <SelectItem key={a.value} value={a.value}>{a.label} · {a.role}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <p className="text-xs text-muted-foreground">
                Kosongkan agar sistem membagi lead otomatis ke sales yang aktif (round-robin).
              </p>
            </div>
          ) : null}
          <div className="space-y-1.5 sm:col-span-2">
