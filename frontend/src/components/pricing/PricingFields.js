          ...
          <SelectContent>
            {schemes.map((s) => (
              <SelectItem key={s.id} value={s.id}>
                {s.name}{s.kind_label ? ` · ${s.kind_label}` : ""}{s.is_default ? " · bawaan" : ""}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
        {(() => {
          const sel = schemes.find((s) => s.id === form.scheme_id);
          if (!sel) {
            return (
              <p className="text-[11px] text-muted-foreground">
                Skema yang dipilih di sini menjadi skema KONTRAK dan dasar termin tagihan (AR) pembeli.
              </p>
            );
          }
          return (
            <div data-testid={CHUB.schemePreview} className="rounded-md border bg-secondary/40 px-2.5 py-2 text-xs">
              <p className="font-medium">
                {sel.name}
                {sel.kind_label ? <span className="ml-1.5 rounded-full bg-primary/10 px-2 py-0.5 text-[11px] text-primary">{sel.kind_label}</span> : null}
              </p>
              {(sel.items || []).length ? (
                <ul className="mt-1 space-y-0.5">
                  {sel.items.map((it, i) => (
                    <li key={i} data-testid={CHUB.schemePreviewRow} className="flex justify-between gap-2 text-muted-foreground">
                      <span>{i + 1}. {it.label}</span>
                      <span className="tabular-nums">
                        {it.basis === "percent" ? `${it.value}%` : (it.value != null ? formatIDR(it.value) : "")}
                        {it.due_rule ? ` · ${it.due_rule}` : (it.due_offset_days != null ? ` · H+${it.due_offset_days}` : "")}
                      </span>
                    </li>
                  ))}
                </ul>
              ) : <p className="mt-1 text-muted-foreground">Termin skema ini belum dikonfigurasi.</p>}
              {sel.kind === "kpr" ? (
                <p className="mt-1 text-amber-800">Skema KPR: kontrak akan membuka sub-alur KPR (SP3K, akad kredit, pencairan bank).</p>
              ) : null}
            </div>
          );
        })()}
