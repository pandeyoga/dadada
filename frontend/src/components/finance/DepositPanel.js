...
function ReceiveDepositDialog({ open, onOpenChange, ars, onDone }) {
  ...
  useEffect(() => { if (open) { setDealId(""); setAmount(""); setNote(""); setProofFiles([]); } }, [open]);
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      await api.post(`/finance/ar/${dealId}/deposit`, { amount: amt, note: note || null, proof_file_ids: proofFiles });
      ...
      <DialogContent>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1.5">
            ...
            <EvidenceUploader value={proofFiles} onChange={setProofFiles} required ownerType="receipt_proof"
              ownerId={dealId || null} testId="deposit-proof-input" label="Bukti bayar" />
          </div>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)} disabled={busy}>Batal</Button>
          <Button data-testid={FINANCE.depositReceiveSubmit} onClick={submit} disabled={busy || !proofFiles.length}>
