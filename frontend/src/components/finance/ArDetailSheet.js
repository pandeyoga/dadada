                  ...
                  <div key={it.id} data-testid="ar-item" data-basis={it.basis} data-payer={it.payer || "buyer"} data-kpr-excluded={it.kpr_excluded ? "1" : "0"}
                    ...
                    <div>
                      <p className="font-medium">{it.label}
                        ...
                        {it.kind === "kpr_shortfall" ? (
                          <span data-testid="ar-item-kpr-shortfall" title={it.note || ""} className="ml-1.5 rounded-full bg-amber-100 px-1.5 text-[10px] font-normal text-amber-800">selisih plafon SP3K · kewajiban pembeli</span>
                        ) : null}
                        {it.kind === "kpr_shortfall" ? (
                          <span data-testid="ar-item-kpr-shortfall" title={it.note || ""} className="ml-1.5 rounded-full bg-amber-100 px-1.5 text-[10px] font-normal text-amber-800">selisih plafon SP3K · kewajiban pembeli</span>
                        ) : null}
