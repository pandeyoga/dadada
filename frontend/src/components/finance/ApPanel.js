...
import { Receipt, Plus, CheckCircle2, Banknote, Eye } from "lucide-react";
...
import ApBillDetailSheet from "@/components/finance/ApBillDetailSheet";
...
export default function ApPanel() {
  ...
  return (
    <div data-testid={FINANCE.apPanel} className="space-y-5">
              ...
              {bills.map((b) => (
                <TableRow key={b.id} data-testid={FINANCE.billRow}>
                  <TableCell className="font-medium">
                    <button type="button" data-testid="ap-bill-detail-btn" className="text-left hover:underline"
                      onClick={() => setDetailId(b.id)}>{b.vendor}</button>
                    {b.note ? <p className="max-w-[260px] truncate text-[11px] text-muted-foreground">{b.note}</p> : null}
                  </TableCell>
                  ...
                  <TableCell className="col-actions">
                    <div className="flex justify-end gap-1.5">
                      <Button size="sm" variant="ghost" data-testid="ap-bill-detail-icon" aria-label="Detail tagihan"
                        onClick={() => setDetailId(b.id)}><Eye className="h-3.5 w-3.5" /></Button>
      ...
      <ApBillDetailSheet billId={detailId} open={!!detailId} onOpenChange={(v) => !v && setDetailId(null)} />
