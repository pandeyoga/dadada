...
export default function CostBillingPanel({ contract, onChanged }) {
  ...
  const pay = () => {
    if (!(form.proof_file_ids || []).length) { toast.error("Bukti pembayaran wajib dilampirkan."); return; }
    return run(() => api.post(`/cost-invoices/${dlg.inv.id}/pay`, { amount: Number(form.amount) || 0, method: form.method || "transfer", note: form.note, proof_file_ids: form.proof_file_ids || [] }), "Kuitansi biaya terbit → titipan bertambah.");
  };
        ...
        <DialogContent className="max-w-md bg-background">
          ...
          <div className="space-y-2">
            ...
            {view?.kind === "pay" ? (<>
              ...
              <EvidenceUploader value={form.proof_file_ids || []} onChange={(v) => setForm((f) => ({ ...f, proof_file_ids: v }))} required
          ...
          <DialogFooter>
            ...
            <Button data-testid={view?.kind === "pay" ? P75.costPaySubmit : view?.kind === "disburse" ? P75.costDisburseSubmit : P75.costExpenseSubmit}
              disabled={busy || (view?.kind === "pay" && !(form.proof_file_ids || []).length)}
