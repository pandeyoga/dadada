...
export default function KprDisbursementBox({ contract, app, mayUpdate, onChanged, onRecord }) {
  ...
  const hasActiveDisb = list.some((d) => d.status !== "dibatalkan");
  ...
  return (
    <div data-testid="kpr-disbursement-box" className="space-y-2 rounded-lg border bg-background p-3">
      ...
      {!tranches.length || (mayUpdate && !hasActiveDisb && changing) ? (
        mayUpdate ? (
          <div className="flex gap-2">
            ...
            {changing ? <Button size="sm" variant="ghost" onClick={() => setChanging(false)}>Batal</Button> : null}
          </div>
        ) : <p className="text-xs text-muted-foreground">Skema pencairan belum dipilih.</p>
      ) : (
        <>
          <p className="flex items-center justify-between text-xs text-muted-foreground" data-testid="kpr-disb-scheme-name">
            <span>Skema: <b>{app.disbursement_scheme_name}</b>{app.disbursement_scheme_auto ? " · dipasang otomatis dari konfigurasi" : ""} · toleransi ±{app.disbursement_tolerance_pct || 0}%</span>
            {mayUpdate && !hasActiveDisb ? (
              <Button data-testid="kpr-disb-scheme-change" size="sm" variant="ghost" className="h-6 px-2" onClick={() => setChanging(true)}>Ganti skema</Button>
            ) : null}
          </p>
