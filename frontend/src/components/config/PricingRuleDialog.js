...
const EMPTY = {
  ...
  requires_approval: false, approval_mode: "global", stackable: true, quota_total: 0, quota_per_customer: 1,
  target: "price", target_component: "",
};

export const APPROVAL_MODE_LABEL = {
  global: "Ikut ambang organisasi (persen & nominal)",
  always: "Selalu perlu persetujuan manajer",
  never: "Tidak pernah (dikecualikan dari ambang)",
};
...
export default function PricingRuleDialog({ kind, source, open, onOpenChange, onSaved }) {
  ...
  useEffect(() => {
    ...
    setForm({ ...EMPTY, ...(source || {}), target: source?.target || "price",
      approval_mode: source?.approval_mode || (source?.requires_approval ? "always" : "global"),
  ...
  const submit = async () => {
    ...
    try {
      const body = {
        ...
        approval_mode: form.approval_mode || "global",
        requires_approval: form.approval_mode === "always",
        ...
        <div className="max-h-[62vh] space-y-3 overflow-y-auto pr-1">
          ...
          <div className="space-y-1.5">
            ...
            <Select value={form.approval_mode || "global"} onValueChange={(v) => patch({ approval_mode: v, requires_approval: v === "always" })}>
              ...
              <SelectContent>
                {Object.entries(APPROVAL_MODE_LABEL).map(([k, v]) => (
                  <SelectItem key={k} value={k} data-testid={`${PRICING.formApproval}-opt-${k}`}>{v}</SelectItem>
                ))}
              </SelectContent>
            </Select>
            <p className="text-[11px] text-muted-foreground" data-testid="pricing-rule-form-approval-help">
              Berlaku untuk penawaran DAN reservasi/SPR. Ambang organisasi (batas persen sales & ambang nominal) diatur di tab <b>Penawaran</b> Pusat Konfigurasi.
            </p>
          </div>
