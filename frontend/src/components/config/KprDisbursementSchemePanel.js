...
export default function KprDisbursementSchemePanel() {
  ...
  const totalPct = (form?.tranches || []).filter((t) => !Number(t.amount)).reduce((s, t) => s + (Number(t.pct) || 0), 0);
  const totalFixed = (form?.tranches || []).reduce((s, t) => s + (Number(t.amount) || 0), 0);
      ...
      <Dialog open={!!form} onOpenChange={(v) => !v && setForm(null)}>
        <DialogContent className="max-w-2xl bg-background">
          <DialogHeader><DialogTitle>{form?.id ? "Ubah" : "Tambah"} skema pencairan</DialogTitle>
            <DialogDescription>Tahap bernominal = retensi tetap (mis. BN sertifikat, listrik). Tahap ber-% membagi SISA plafon setelah retensi — totalnya harus 100%. Bila semua tahap bernominal, tahap pertama menyerap selisih terhadap plafon SP3K kontrak.</DialogDescription></DialogHeader>
          {form ? (
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                <Label>Tahapan · tahap ber-% total {totalPct}%{totalFixed ? ` · retensi tetap Rp ${totalFixed.toLocaleString("id-ID")}` : ""}</Label>
