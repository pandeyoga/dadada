  ...
  return (
    <div className="space-y-3">
      ...
      <Dialog open={!!form} onOpenChange={(v) => !v && setForm(null)}>
        <DialogContent data-testid="allin-scheme-dialog" className="max-w-4xl bg-background">
          ...
          <DialogFooter><Button variant="outline" onClick={() => setForm(null)}>Batal</Button>
            <Button data-testid={P75.schemeSave} disabled={busy || !form?.name?.trim() || !form?.items?.length || form?.items?.some(it => !it.component_code)} onClick={save}>Simpan</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
