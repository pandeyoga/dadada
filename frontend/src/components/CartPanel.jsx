...
export function CartPanel({
  ...
  // KN-C06 — satuan yang faktornya tidak dikenal layar TIDAK boleh lolos diam-diam
  // sebagai 1× (harga layar bisa 50× lebih kecil dari yang disimpan server).
  const unknownUnitLines = cart.filter((item) => {
    const u = item.unit || item.product?.base_unit;
    return u && convFactor(item.product, u) == null;
  });
  const unitBlocked = unknownUnitLines.length > 0;
  // KN-C07 — aturan minimum potong (`inventory.min_cut_qty`) dulu HANYA di server dan baru
  // menolak pada tombol terakhir; kini layar memberi tahu sejak kuantitas diketik.
  const minCut = Number(settings?.inventory?.min_cut_qty) || 0;
  const shortCutLines = minCut > 0 ? cart.filter((item) => {
    if (item.purchase_mode === "roll") return false;
    const f = convFactor(item.product, item.unit || item.product?.base_unit) ?? 1;
    const baseQty = (Number(item.quantity) || 0) * f;
    return baseQty > 0 && baseQty < minCut;
  }) : [];
  const minCutBlocked = shortCutLines.length > 0;
  ...
  return (
    <section data-testid="cart-panel" className="section-card">
      ...
      <div className="section-body">
        ...
        {minCutBlocked && (
          <div data-testid="cart-min-cut-warning" className="mt-2 rounded border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-800">
            Di bawah minimum potong ({minCut} {shortCutLines[0]?.product?.base_unit || "meter"}): {shortCutLines.map((i) => `${i.product?.name || i.product?.sku} (${i.quantity} ${i.unit || i.product?.base_unit})`).join(", ")}.
            Naikkan kuantitas sebelum menyimpan pesanan.
          </div>
        )}
        {unitBlocked && (
          <div data-testid="cart-unknown-unit-warning" className="mt-2 rounded border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-800">
            Faktor konversi satuan belum dikenal untuk: {unknownUnitLines.map((i) => `${i.product?.name || i.product?.sku} (${i.unit})`).join(", ")}.
            Harga di layar tidak bisa dihitung — muat ulang katalog satuan sebelum menyimpan pesanan.
          </div>
        )}
        <button
          data-testid="submit-sales-order-button"
          className="primary-button mt-2 w-full"
          disabled={!canWrite || !selectedCustomer || !selectedAddress || cart.length === 0 || lotPlanLoading || creditBlocked || unitBlocked || minCutBlocked}
          title={unitBlocked ? "Ada satuan yang faktor konversinya belum dikenal layar — muat ulang katalog satuan." : minCutBlocked ? `Ada baris di bawah minimum potong ${minCut} ${shortCutLines[0]?.product?.base_unit || ""}.` : writeBlockHint}
