    ...
    <div data-testid={PORTAL.paymentsPanel} className="space-y-6">
      {data.map((p) => (
        <div key={p.deal_id} className="space-y-3">
          ...
          <PortalBookingFeeCard bf={p.booking_fee} dealId={p.deal_id} unitCode={p.unit_code}
            onProof={(d) => setProofDeal({ ...d, booking_fee: true })} />
      ...
      <PaymentProofDialog open={!!proofDeal} onOpenChange={(v) => !v && setProofDeal(null)}
        ...
        endpoint={proofDeal?.booking_fee ? "/portal/booking-fee/proof" : "/portal/payments/proof"}
        title={proofDeal?.booking_fee ? "Kirim bukti transfer booking fee" : "Kirim bukti transfer"} />
    </div>
  );
}
