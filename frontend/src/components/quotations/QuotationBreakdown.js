      ...
      <div className="rounded-lg border bg-card p-3 shadow-[var(--shadow-card)]">
        ...
        <div className="mt-1.5 divide-y">
          <Line label={`Harga unit ${calc.unit?.code || calc.unit_code || ""}`}
            value={calc.base_price}
            hint={calc.price_source === "skema"
              ? `${calc.unit?.type ? `${calc.unit.type} · ` : ""}harga khusus skema ${calc.scheme?.name || calc.price_scheme_kind || ""}${calc.list_price && calc.list_price !== calc.base_price ? ` (harga dasar ${new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(calc.list_price)})` : ""}`
              : calc.unit?.type} />
